...
if (!document.querySelector('.eliura_container')) {
  var exports = {}
  exports.EliuraOptions = exports.XPath = exports.Eliura = void 0
  ...
  var Eliura = /*#__PURE__*/ (function () {
    function Eliura(options) {
      _classCallCheck(this, Eliura)

      _defineProperty(this, 'attributePriorizationList', [
        'name',
        'class',
        'title',
        'alt',
      ...
      _defineProperty(this, 'attributeBlackList', [
        'href',
        'src',
        'onclick',
        'onload',
    ...
    _createClass(Eliura, [
      ...
      },
      {
        ...
        value: function transfConvertStar(xPath, element) {
          ...
          if (xPath.startsWith('//*')) {
            output.push(
              new XPath(
                '//' + ancestor.tagName.toLowerCase() + xPath.substring(3)
              )
            )
          }
        ...
        value: function transfAddText(xPath, element) {
          ...
          if (
            ...
            newXPath.addPredicateToHead(
              "[contains(text(),'".concat(ancestor.textContent, "')]")
            )
            output.push(newXPath)
          }
          ...
          if (!xPath.headHasAnyPredicates()) {
            ...
            try {
              for (_iterator3.s(); !(_step3 = _iterator3.n()).done; ) {
                ...
                if (
                  !this.attributeBlackList.includes(_attribute.name) &&
                  !this.attributePriorizationList.includes(_attribute.name)
                ) {
                  var _newXPath = new XPath(xPath.getValue())

                  _newXPath.addPredicateToHead(
                    '[@'
                      .concat(_attribute.name, "='")
                      .concat(_attribute.value, "']")
                  )
      ...
      },
      {
        ...
        value: function transfAddAttributeSet(xPath, element) {
          ...
          if (!xPath.headHasAnyPredicates()) {
            ...
            attributePowerSet = attributePowerSet.filter(function (
              attributeSet
            ) {
            ...
            try {
              for (_iterator6.s(); !(_step6 = _iterator6.n()).done; ) {
                ...
                var predicate = '[@'
                  .concat(_attributeSet[0].name, "='")
                  .concat(_attributeSet[0].value, "'")

                for (var i = 1; i < _attributeSet.length; i++) {
                  predicate += ' and @'
                    .concat(_attributeSet[i].name, "='")
                    .concat(_attributeSet[i].value, "'")
                }
      ...
      },
      {
        ...
        value: function generatePowerSet(input) {
          return input.reduce(
            function (subsets, value) {
              return subsets.concat(
                subsets.map(function (set) {
  ...
  exports.EliuraOptions = EliuraOptions

  // END - Main Functionality on DOM

  /////////// Init Function
  document.querySelectorAll('noscript').forEach(function (item) {
    item.remove()
  })

  var styleItem =
    '.eliura-hover{outline:2px dashed #ffafaf!important;cursor:pointer;}.eliura_container{position:fixed;top:0;left:0;z-index:9999;width:20%;transform: translateX(-100%);transition: all 150ms;}.eliura_container.open{transform: translateX(0);}.eliura_toggle{line-height:14px;direction:rtl;position:absolute;width:30px;height:100px;right:-30px;top:calc(50% - 75px);background:#00adff;border-radius:0 20px 20px 0;writing-mode:vertical-rl;padding:0 9px;font-weight:bold;box-sizing:border-box;font-family:sans-serif;text-align:center;font-size:16px;letter-spacing:2px;color:#fff;cursor:pointer;}.eliura_toggle:hover{background: #60cbff;}.eliura_list{width:100%;height:100vh;overflow-y:auto;background:#ddf4ff;z-index:9999;padding:15px 10px 35px;box-sizing:border-box}.eliura_item{position:relative;width:100%;background:#00adff;display:inline-block;height:50px;margin-bottom:5px}.remove_btn{margin:0;background:#989898;font-size:40px;color:#fff;height:100%;width:30px;text-align:center;position:absolute;right:0;display:inline-flex;justify-content:center;align-items:center;top:0;box-sizing:border-box;line-height:0.8;font-family:sans-serif;cursor:pointer;}.remove_btn:hover{background:#00adff;}.item_title{margin:0;direction:ltr;left:0;position:absolute;font-family:sans-serif;line-height:2.5;padding-left:8px;width:88%;display:inline-block;border:1px solid #fff;height:50%;background:white;box-sizing:border-box;}.item_title:nth-child(2){top:50%;}.eliura_export{cursor:pointer;position:absolute;bottom:0;background:#f63d4b;width:100%;height:35px;text-align:center;box-sizing:border-box;color:#fff;font-size:18px;font-family:sans-serif;line-height:35px}.eliura_export:hover{background:#d41725;}'
  var style = document.createElement('style')
  style.innerHTML = styleItem
  document.head.appendChild(style)

  // Add Eliura UI on Page
  ...
  document.body.appendChild(eliura_container)

  // Local Storage Management

  var localStorageName = 'eliura_elements'

  var removeSelectedElement = function (number) {
    var elements = JSON.parse(window.localStorage.getItem(localStorageName))
      .elements
  ...
  var addSelectedElement = function (target) {
    var isNew = true
    ...
    if (isNew) {
      var ecs = getECSPath(jQuery(target), '')
  ...
  var updateSelectedElements = function (elements) {
    var data = JSON.parse(window.localStorage.getItem(localStorageName))
    ...
    elements.forEach(function (element, index) {
      ...
      var item_title = document.createElement('input')
      item_title.classList.add('item_title')
      item_title.value = element.cssSelector
      eliura_item.appendChild(item_title)
      item_title.setAttribute('readonly', '')
  ...
  var checkSpecificElement = function (event) {
    var classList = event.target.classList
    return (
      // classList.contains('eliura-hover') ||
      classList.contains('eliura_container') ||
      classList.contains('eliura_export') ||
      classList.contains('eliura_list') ||
      classList.contains('eliura_toggle') ||
      classList.contains('eliura_item') ||
      classList.contains('item_title') ||
      classList.contains('remove_btn')
    )
  }
  ...
  document.onmousemove = function (event) {
    if (!checkSpecificElement(event)) {
      ...
      for (var i = 0; i < 20; i++) {
        if (parent != null) {
          parent.classList.remove('eliura-hover')
...
function getECSPath(element, prev) {
  ...
  else className = '.' + className.split(' ').join('.')

  var tagName = element.prop('tagName')
  if (tagName == undefined) return prev

  if (prev != '') prev = tagName + id + className + ' ' + prev
  else prev = tagName + id + className

  if (element.parent().length > 0) {
    return getECSPath(element.parent(), prev)
  } else {
    return prev
  }
}
